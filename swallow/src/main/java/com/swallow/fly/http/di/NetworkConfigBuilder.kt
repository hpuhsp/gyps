...
class NetworkConfigBuilder {
    ...
    /**
     * 自定义成功判断逻辑，优先级高于 BaseResponse 默认实现。
     * 参数：(code: Int, status: String) -> Boolean
     */
     ...
     * 示例：
     * ```kotlin
     * builder.successChecker { code, status -> code == 200 || status == "ok" }
     * ```
     *
     * @param checker (code, status) -> Boolean，返回 true 表示业务成功
     * @return 当前构建器，支持链式调用
     */
