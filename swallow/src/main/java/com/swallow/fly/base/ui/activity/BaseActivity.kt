...
abstract class BaseActivity<VM : BaseViewModel, VB : ViewBinding> :
     ...
     * ```
     *
     * 推荐使用 Hilt 时直接用 `by viewModels()` 替代此属性。
     */
    @Suppress("UNCHECKED_CAST")
    open val modelClass: Class<VM> by lazy {
        resolveViewModelClass(javaClass) as Class<VM>
    }
...
/**
 * 通过反射从泛型签名中推断第一个类型参数对应的 Class。
 * 用于 BaseActivity / BaseFragment 自动解析 ViewModel Class，避免子类强制声明 modelClass。
 *
 * 遍历继承链，找到第一个泛型参数为具体 Class（非 TypeVariable）的层级。
 * 这样可以正确处理中间有抽象父类的情况。
 */
internal fun resolveViewModelClass(clazz: Class<*>): Class<*> {
    ...
    while (current != Any::class.java) {
        ...
        if (genericSuperclass is java.lang.reflect.ParameterizedType) {
            val typeArg = genericSuperclass.actualTypeArguments.firstOrNull()
            // 只接受具体 Class，跳过 TypeVariable（未解析的泛型参数）
            if (typeArg is Class<*>) return typeArg
        }
        current = current.superclass ?: break
    }
    throw IllegalStateException(
        "Cannot resolve ViewModel class for ${clazz.simpleName}. " +
        "Please override modelClass manually."
    )
}
